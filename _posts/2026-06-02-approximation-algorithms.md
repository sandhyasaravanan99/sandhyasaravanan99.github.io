...
We need examples. Let's examine a famous example, the Minimum Vertex Cover problem aka one of the problems that made me zone out of TCS lectures because of my tubelight brain. The formulation of the Minimum Vertex Cover problem is as follows:
...
The constraint $$x_i + x_j \geq 1$$ says that at least one endpoint of each edge is chosen.
...
This is the **linear programming (LP) relaxation**, and unlike the integer version, it *can* be solved efficiently. Of course, the LP might hand us fractional answers like $$x_i = \tfrac{1}{2}$$, which makes no sense for "is vertex $$i$$ in the cover or not?" But notice that every valid integer solution is also a valid solution to the relaxed problem, so the LP's optimum can only be *smaller* than (or equal to) the true integer optimum.
...
For every selected vertex $$i \in C$$, we have $$x_i^\star \geq 1/2$$, so
...
$$
|C| = \sum_{i \in C} 1 \leq \sum_{i \in C} 2x_i^\star \leq 2 \sum_{i \in V} x_i^\star = 2\,\text{LP}^\star.
$$

Since $$\text{LP}^\star \leq \text{OPT}$$,
